# L'array avec lequel nous travaillons
...
def shortest_handle
    puts $arr.sort_by(&:length)[0]
end

#Combien y-a-t'il de handle contenant 5 caractères (le @ ne compte pas pour un caractère)
def five_char_handle
    puts $arr.select{ |elt| elt.length == 6}.length
end

#Combien commencent par une majuscule (première lettre juste après le @) ?
def capital_handle
    puts $arr.select{ |elt| elt[1] =~ /[A-Z]/}.length
end
...
def handle_size_table
    handle_max = $arr.sort_by(&:length).last.length
    for i in (2..handle_max)
    puts "Il y a #{$arr.select{ |elt| elt.length == i}.length} handles avec #{i-1} caractères"
    end
end

puts "Bienvenus dans The Super Array !"
puts "============================================================"
puts "Entre le chiffre correspondant pour naviguer dans le menu :"
puts "============================================================"
puts "Combien y a-t-il de handle dans cette array ? : 1"
puts "Quel est le handle le plus court de cette liste ? : 2"
puts "Combien y-a-t'il de handle contenant 5 caractères ? : 3"
puts "Combien commencent par une majuscule (première lettre juste après le @) ? : 4"
puts "Trie la liste de handle par ordre alphabétique : 5"
puts "Trie la liste de handle par taille des handle : 6"
puts "Quelle est la position dans l'array de la personne @epenser ? : 7"
puts "Sors-moi une répartition des handle par taille de ces derniers : 8"
puts "============================================================"
...
def menu
    ...
    when "8"
        ...
        puts "Entre un chiffre entre 1 et 8 pour naviguer dans le menu"
        menu
    end
end
